# Chaos Ball

## 项目简介
本项目是《虚拟现实开发引擎》实验2的实践产物：**Chaos Ball**。这是一款快节奏的街机风格弹球游戏，结合了 Pinball 和 Breakout 的部分玩法。玩家在一个封闭的舞台中，需要利用自己的“减震器”将四种不同颜色的彩球（红、蓝、橙、绿）分别撞入对应颜色的角落球门中。同时，舞台中还有许多快速移动的黄色“混乱球（Chaos Ball）”对玩家进行干扰。

## 游戏规则
- **目标**：将所有 4 只彩色球分别推入对应颜色的角落球门。当 4 只球都进入正确的球门时，玩家获胜。
- **干扰**：舞台上的黄色“混乱球”会快速移动并干扰玩家和彩球。
- **特性**：
  - 玩家和球都不能离开舞台。
  - 所有对象均具有“超级弹性”，碰撞不会损失动量（通过自定义的物理材质实现）。
  - 彩球和混乱球的初始速度和方向是随机的。
  - 游戏不存在失败条件，只有完成时间的挑战。

## 目录结构 (Assets/)
- `Materials/`: 包含游戏中使用的各种材质，如墙壁材质、不同颜色的球材质（红、蓝、绿、橙、黄）以及自定义的超级弹性物理材质 `SuperBouncy.physicMaterial`。
- `Prefabs/`: 存放预制体，如墙壁 `Wall.prefab`，方便重复使用。
- `Scenes/`: 包含游戏的主场景 `SampleScene.unity`。
- `Scripts/`: 包含游戏的核心逻辑脚本：
  - `VelocityScripts.cs`: 赋予球体随机初始速度并限制其移动轴。
  - `GoalScripts.cs`: 附加在球门上，检测是否有对应颜色的彩球进入触发器。
  - `GameControlScripts.cs`: 游戏总控脚本，检查所有球门的状态以判定游戏是否获胜并显示 GUI 提示。
- `StandardAssets/`: Unity 标准资源包，包含第一人称角色控制器 (`Characters`)、环境资源 (`Environment`) 等。

## 开发技术点
- **场景构建**：使用地形(Terrain)和基本几何体(Cube)搭建游戏舞台，并进行纹理贴图。
- **物理系统**：运用刚体(Rigidbody)、碰撞器(Collider)和触发器(Trigger)。自定义 `Physic Material` 实现无动能损耗的完美反弹。
- **角色控制**：利用 Unity Standard Assets 中的 First Person Controller 进行改造，增加用于击球的“减震器(Bumper)”。
- **脚本编程**：使用 C# 编写游戏逻辑，包括随机速度生成、触发器检测、游戏状态管理和简单的 UI 界面显示。

## 运行环境
- Unity 引擎 (推荐使用与项目一致的版本)
